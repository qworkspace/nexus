...
export default function MeetingsPage() {
  ...
  const meetingActions = selectedId
    ? actionItems.filter(i => {
        const src = typeof i.source === 'string' ? i.source : (i.source?.meeting || i.source?.date || '');
        return src === selectedId || src.includes(selectedMeeting?.date || '____');
      })
    : [];
        ...
        <div className="lg:col-span-1 bg-white dark:bg-card rounded-xl border border-zinc-200 dark:border-border p-3 max-h-[80vh] overflow-y-auto">
          <div className="space-y-1">
            {topicFiltered.map(m => {
              ...
              return (
                ...
                >
                  ...
                  <p className={`text-[10px] mt-0.5 ${selectedId === m.id ? "text-foreground" : "text-muted-foreground"}`}>
                    {m.date}
                  </p>
                  {topics.length > 0 && (
                    <div className="flex gap-1 flex-wrap mt-1.5">
                      ...
                      {topics.length > 3 && (
                        <span className={`text-[8px] ${selectedId === m.id ? "text-foreground" : "text-muted-foreground"}`}>
                          +{topics.length - 3}
                        </span>
                      )}
                    </div>
            ...
            {topicFiltered.length === 0 && (
              <p className="text-muted-foreground text-xs text-center py-4">No meetings found</p>
            )}
          </div>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="bg-white dark:bg-card rounded-xl border border-zinc-200 dark:border-border p-5">
            {content ? (
              <div>
                ...
                {summary && (
                  <div className="mb-3 p-3 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                    <h3 className="text-xs font-bold text-amber-900 dark:text-[#FFE135] mb-1.5 flex items-center gap-1">
                      <FileText size={12} />
                      TL;DR
                    </h3>
                  ...
                  <div className="mb-3">
                    <div className="flex gap-1 flex-wrap">
                      {meetingTopics[selectedId!].map(t => (
                        <span key={t} className="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-foreground rounded-full text-[10px]">
                          {t}
                        </span>
                      ))}
                    </div>
              ...
              <p className="text-muted-foreground text-center py-8 text-xs">Select a meeting to view its transcript</p>
            )}
          </div>
          ...
          {selectedId && (
            <div className="bg-white dark:bg-card rounded-xl border border-zinc-200 dark:border-border p-5">
              ...
              {meetingActions.length > 0 ? (
                ...
                <p className="text-[11px] text-muted-foreground mb-4">No action items from this meeting</p>
              )}
    ...
    <div className={`p-2.5 rounded-lg border border-zinc-200 dark:border-border border-l-2 ${statusColors[item.status] || "border-l-zinc-400"} bg-zinc-50 dark:bg-secondary/50`}>
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {item.status === "new" && (
            <button onClick={() => onUpdateStatus(item.id, "in-progress")} className="text-[10px] text-foreground hover:underline">
              Start →
            </button>
          )}
          {item.status === "in-progress" && (
            <button onClick={() => onUpdateStatus(item.id, "done")} className="text-[10px] text-foreground hover:underline flex items-center gap-0.5">
              Done <Check size={10} />
            </button>
          )}
          {item.status === "blocked" && (
  ...
  return (
    <div className="space-y-1.5">
      {lines.map((line, i) => {
        ...
        if (line.startsWith("- [x] ")) return <div key={i} className="flex items-center gap-1.5 text-xs"><CheckSquare size={11} className="text-green-600" /><span className="text-muted-foreground line-through">{line.slice(6)}</span></div>;
