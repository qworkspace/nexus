      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg font-semibold flex items-center gap-2">
            <Activity className={`h-5 w-5 ${allHealthy ? "text-zinc-600" : hasCritical ? "text-red-500" : "text-yellow-500"}`} />
            HEALTH MONITOR
          </CardTitle>
        </CardHeader>
        <CardContent>
          ...
          {/* All healthy message */}
          {alerts.length === 0 && (
            <div className="mb-4 p-3 bg-zinc-50 dark:bg-green-950 border border-zinc-300 dark:border-zinc-300 rounded-lg">
...
function AlertBadge({ alert }: AlertBadgeProps) {
  const getStyles = (type: string) => {
    switch (type) {
      ...
      default:
        return {
          bg: "bg-zinc-50 dark:bg-blue-950",
          border: "border-zinc-300 dark:border-zinc-300",
