...
export default function ResearchPage() {
  ...
  const getSourceConfig = (source: string) => {
    return SOURCE_CONFIG[source] || { label: source, color: 'text-zinc-700 dark:text-zinc-400', bgColor: 'bg-zinc-100 dark:bg-zinc-800' };
  };
  ...
  return (
    <>
      <div className="p-6 max-w-6xl">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                title="Refresh"
              >
                <Download className={`h-4 w-4 text-zinc-600 dark:text-zinc-400 ${loading ? 'animate-spin' : ''}`} />
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6 relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 border border-zinc-200 dark:border-zinc-700 rounded-md bg-white dark:bg-zinc-900 text-sm text-zinc-900 dark:text-zinc-100"
          />
        </div>
        ...
        <div className="mb-6 flex flex-wrap gap-2">
          <button
            className="px-3 py-1.5 text-sm font-medium rounded-lg transition-all bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 shadow"
          >
            All Sources
          </button>
          {Object.entries(SOURCE_CONFIG).slice(0, 4).map(([key, config]) => (
            <button
              key={key}
              className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-100 dark:hover:bg-zinc-800 ${config.color}`}
            >
              {config.label}
            </button>
          ))}
      ...
      <div
        ref={panelRef}
        className={`fixed top-0 right-0 h-full w-full max-w-md bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-700 z-50 transform transition-transform duration-300 ease-out ${
        ...
        {selectedItem && (
          <div className="h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
              {/* Title */}
              <div>
                <h3 className="text-xl font-semibold text-zinc-900 dark:text-zinc-100">{selectedItem.title}</h3>
              </div>
            ...
            <div className="p-4 border-t border-zinc-200 dark:border-zinc-700">
              <button
                onClick={closeSlideOver}
                className="w-full py-2 px-4 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 rounded-lg text-sm font-medium text-zinc-700 dark:text-zinc-300 transition-colors"
              >
                Close
              </button>
            </div>
...
function ResearchCard({ 
  ...
  return (
    <Card className="hover:shadow-md transition-all cursor-pointer group hover:border-zinc-300 dark:hover:border-zinc-600" onClick={onClick}>
      <CardHeader>
        <div className="flex items-start justify-between">
          <div>
            <CardTitle className="text-lg text-zinc-900 dark:text-zinc-100">{item.title}</CardTitle>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              {item.critical.slice(0, 3).map((item, i) => (
                <p key={i} className="text-sm text-zinc-700 dark:text-zinc-300 line-clamp-1">
                  {item}
                </p>
              ))}
            </div>
