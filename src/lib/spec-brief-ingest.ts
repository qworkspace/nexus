/**
 * Shared spec-brief ingestion logic.
 * Used by both the manual POST /api/pipeline-queue/ingest endpoint
 * and the automatic file watcher.
 */
...
export const SPEC_BRIEFS_DIR = join(homedir(), '.openclaw', 'shared', 'research', 'ai-intel', 'spec-briefs');
...
export type Source = 'research' | 'deep-focus' | 'innovation-think' | 'retro' | 'pj-request' | 'manual';

export function mapSource(sourceStr: string): Source {
...
export function safeDate(v: string | undefined): Date {
...
export function parseMdBrief(content: string, filename: string): ParsedBrief {
  ...
  return {
    ...
    complexity: ['HIGH', 'MED', 'LOW', 'SMALL'].includes(complexity) ? complexity : 'MED',
...
export function parseJsonBrief(data: Record<string, unknown>, filename: string): ParsedBrief {
...
export interface PreviewResult {
  preview: { id: string; title: string; file: string; priority: string }[];
  total: number;
}
 ...
 */
export async function previewIngest(): Promise<PreviewResult> {
  ...
  return { preview, total: preview.length };
}

/**
 * Ingest a single spec-brief file into the database.
 * Returns the brief ID if ingested, null if skipped (already exists or error).
 */
export async function ingestSingleFile(file: string): Promise<string | null> {
  const filePath = join(SPEC_BRIEFS_DIR, file);
  const isMd = file.endsWith('.md');
  const isJson = file.endsWith('.summary.json');
  ...
  try {
    ...
    // Also check by slug similarity to avoid near-duplicates
    const existingBySlug = await db.brief.findMany({ select: { id: true } });
    const existingSlugs = new Set(existingBySlug.map(b => {
      const parts = b.id.split('-').slice(4);
      return parts.join('-');
    }));
    ...
    console.error(`[ingest] Failed to ingest ${file}:`, e);
    return null;
  }
}
 ...
 */
export async function ingestAllBriefs(): Promise<IngestResult> {
  if (!existsSync(SPEC_BRIEFS_DIR)) {
    return { ingested: 0, items: [], total: await db.brief.count() };
  }
  ...
  return { ingested, items: newItems, total };
}
